...
<html>
    ...
    <div class="topnav">
        ...
        <a class="refresh" href="optical.php">
    ...
    <?php
        ...
        if ($result = $conn->query($sql)) {
            while ($row = $result->fetch_assoc()) {
                ...
                array_push($row_data, $row["value1"]);
                array_push($row_data, $row["value2"]);
                array_push($row_data, $row["reading_time"]);
                array_push($optical_data, $row_data);
                $row_data = array();
            }
            $result->free();
        }
    ...
    <p class = "table_header"> Optical Flow Sensor Data SQL Table </p>
    <p id = "sqltable"></p>

    <script>
        //acquire sensor data reading from php segment i.e. sql
        const optical_data = <?php echo json_encode($optical_data); ?>;
        //document.getElementById("demo").innerHTML = optical_data.toString();


        function formTable(myArray) {
            var result = "<table>";
            result += "<th> ID </th> <th> X Co-ordinate </th> <th> Y Co-ordinate </th> <th> Timestamp </th>"
        ...
        document.getElementById("sqltable").innerHTML = formTable(optical_data);
